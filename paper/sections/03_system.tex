...
extension sets: 80+ file-extension patterns mapped to 33 language grammars
(e.g., \texttt{.ts}/\texttt{.tsx} $\to$ TypeScript; \texttt{.f}/\texttt{.f90} $\to$ Fortran),
plus PDF,
...
otherwise become pathological hub nodes. Import resolution is language-aware:
...
prevents hub-node explosion while preserving the hub's contextual relevance.
