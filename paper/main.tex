...
\title{SS03: Graphify: Context-Efficient Codebase Question Answering\\
...
answering a question with \textbf{71.5$\times$ fewer tokens} than naive
full-corpus reading and \textbf{5.8$\times$ fewer tokens} than an agentic
grep-and-read baseline on the same 52-file multi-modal corpus.
Graphify integrates with 21 AI assistant platforms and has
accumulated over 1.2M PyPI downloads and 58,600+ GitHub stars.
We report token reduction against both baselines, edge confidence-tag distributions,
and a depth ablation across five real-world corpora,
