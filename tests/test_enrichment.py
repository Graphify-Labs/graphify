...
def test_lsp_summary_supports_generated_skill_get_calls():
    summary = LspEnrichmentSummary(
        enabled=True,
        ran_hooks=("ruby:ruby-lsp",),
        merged_edges=7,
    )

    assert summary.get("hooks_enabled") is True
    assert summary.get("hooks") == ["ruby:ruby-lsp"]
    assert summary.get("merged_edges") == 7
    assert summary.get("missing", "fallback") == "fallback"
