"""PHP `interface` / `trait` / `enum` declarations mint canonical nodes.
...
The control experiment is the shape these tests pin: change
...
def _repro_corpus(tmp_path: Path) -> list[Path]:
    """The 7-file minimal repro, verbatim in shape."""
    return [
        _write(
...
def test_php_interface_declaration_is_the_canonical_edge_target(tmp_path: Path):
    # Criterion 1: `interface FooRepository` mints a sourced declaration node,
    # and implements / imports / references / references_constant all land on it —
    # matching the `class FooRepository` control.
    ...
    # one legitimately dangling target until the import-metadata fix lands — does not.
...
def test_php_trait_and_enum_declared_in_a_differently_named_file(tmp_path: Path):
    # Criterion 2: `trait Loggable` and `enum Status` live in `Extras.php`
    ...
    # target's stub LABEL — skips the edge. That is an independent root cause:
    # the edge must be resolved from its own `target_fqn` metadata. This change's
    # contract here is the half that fix needs and cannot supply: the node exists, is
    # sourced, and carries the FQN the metadata will resolve to.
    #
    # The upstream fork selects this edge by that `target_fqn`; `_import_php` here
    # stamps no metadata on a PHP `imports` edge yet, so it is selected by the bare
    # short-name id it targets instead — the same edge, named the only way this
    # tree can name it.
    enum_import = next(
        e for e in _edges(result, "imports")
        if str(e.get("source_file", "")).endswith("app/Uses/Consumer3.php")
        and e["target"] == _make_id("Status")
    )
    assert enum_import["target"] not in {n["id"] for n in result["nodes"]}, (
        "enum import no longer dangles — the target_fqn resolution has landed; tighten this "
...
def test_php_enum_body_members_and_clauses(tmp_path: Path):
    ...
    # its trait `use` must all be picked up regardless (the enum caveat).
    contract = _write(
...
def test_php_interface_extends_qualified_interface_resolves(tmp_path: Path):
    # Criterion 4: the `_resolve_php_type_references` raw-scan only recognised
