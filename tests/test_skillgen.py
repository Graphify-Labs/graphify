...
def test_always_on_roundtrip_is_byte_faithful():
    ...
    # frozen baseline, with the agents-md/antigravity-rules blocks allowed
    # exactly the sanctioned substitutions recorded in
    # gen.ALWAYS_ON_SANCTIONED_EDITS (#1530, #2006 x2).
    ...
    new_wiki_agents = (
        ...
        "raw source browsing — unless a PreToolUse nudge flags it as predating "
        "graph.json (wiki/ is only regenerated by `graphify export wiki`, never by "
        "`update`/watch), in which case fall back to source or `graphify query`."
    )
    # The sanctioned-edit registry holds exactly these two old->new substitutions
    # for _AGENTS_MD_SECTION: #1530 (install guidance) and #2006 (wiki staleness).
    assert gen.ALWAYS_ON_SANCTIONED_EDITS["_AGENTS_MD_SECTION"] == (
        (old_instruction, new_instruction),
        (old_wiki_agents, new_wiki_agents),
    )
    baseline_agents = gen._always_on_constants(gen.ALWAYS_ON_BASELINE_REF)["_AGENTS_MD_SECTION"]
    # The ONLY divergence from the frozen baseline is the two sanctioned edits —
    # any other byte drift would have surfaced as a problem above.
    assert old_instruction in baseline_agents
    assert old_wiki_agents in baseline_agents
    assert (
        baseline_agents.replace(old_instruction, new_instruction).replace(old_wiki_agents, new_wiki_agents)
        == rendered_agents
    )
    assert "`skill` tool" not in rendered_agents
    assert 'skill: "graphify"' not in rendered_agents

    rendered_antigravity = next(
        a.content
        for a in gen.render_always_on()
        if a.path == "graphify/always_on/antigravity-rules.md"
    )
    old_wiki_antigravity = "- If graphify-out/wiki/index.md exists, navigate it instead of reading raw files"
    new_wiki_antigravity = (
        "- If graphify-out/wiki/index.md exists, navigate it instead of reading raw files — "
        "unless a PreToolUse nudge flags it as predating graph.json (wiki/ is only "
        "regenerated by `graphify export wiki`, never by `update`/watch), in which case "
        "fall back to source or `graphify query`"
    )
    # The sanctioned-edit registry holds exactly this single #2006 substitution
    # for _ANTIGRAVITY_RULES.
    assert gen.ALWAYS_ON_SANCTIONED_EDITS["_ANTIGRAVITY_RULES"] == (
        (old_wiki_antigravity, new_wiki_antigravity),
    )
    baseline_antigravity = gen._always_on_constants(gen.ALWAYS_ON_BASELINE_REF)["_ANTIGRAVITY_RULES"]
    assert old_wiki_antigravity in baseline_antigravity
    assert baseline_antigravity.replace(old_wiki_antigravity, new_wiki_antigravity) == rendered_antigravity
