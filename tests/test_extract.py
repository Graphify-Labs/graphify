...
def test_extract_warns_on_code_files_with_no_ast_extractor(tmp_path, capsys):
    ...
    # of reporting success as if the language were mapped. (.r/.R was this test's
    # example until R gained an extractor.)
    r1 = tmp_path / "analysis.ETS"; r1.write_text("function f(x: number) { return x }\n")
    r2 = tmp_path / "helper.ets"; r2.write_text("function g(y: number) { return y }\n")
    ...
    assert ".ets (2)" in err          # both files grouped under the lowercased ext
    assert "#1689" in err
