...
def test_sourced_node_wins_over_n_sourceless_stubs(stub_count):
    ...
    The guard must not depend on stub counts staying high — a graph built after
    the PHP declaration-node fix has fewer stubs, and the residual ones must
    still be handled.
    """
...
def test_explain_and_affected_agree_when_a_sourced_node_exists(stub_count):
    ...
    answered — that divergence is the reported symptom."""
