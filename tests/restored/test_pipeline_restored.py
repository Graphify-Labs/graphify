...
def run_pipeline(tmp_path: Path) -> dict:
    ...
    # Step 7: export - HTML directly from the native snapshot
...
def test_pipeline_all_nodes_have_community(tmp_path):
    ...
    for node in G.nodes():
        assert node.id in all_community_nodes, f"Node {node.id!r} has no community"
...
def test_pipeline_no_self_loops(tmp_path):
    ...
    G = result["graph"]
    for edge in G.edges():
        assert edge.source != edge.target, f"Self-loop found on node {edge.source!r}"
