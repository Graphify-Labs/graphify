...
def _make_graph():
    return graph_from_payload(
        [
            ...
            {"id": "n4", "label": "stream", "file_type": "code", "source_file": "renderer.py", "community": 1},
        ],
        [
...
def _linked_pair(*, same_community: bool = False):
    return graph_from_payload(
        [
            {"id": "n1", "label": "parse", "file_type": "code", "source_file": "a.py", "community": 0},
            {"id": "n2", "label": "render", "file_type": "code", "source_file": "b.py", "community": 0 if same_community else 1},
        ],
...
def test_community_article_truncation_notice(tmp_path):
    ...
    G = graph_from_payload(
        [{"id": nid, "label": f"concept_{nid}", "file_type": "code", "source_file": "a.py", "community": 0} for nid in nodes],
        [{"source": nodes[i], "target": nodes[i + 1], "relation": "calls", "confidence": "EXTRACTED", "weight": 1.0} for i in range(len(nodes) - 1)],
    )
    communities = {0: nodes}
...
def test_cross_community_links_without_node_community_attrs(tmp_path):
    ...
    G = graph_from_payload(
        [{"id": "n1", "label": "parse", "file_type": "code", "source_file": "parser.py"}, {"id": "n2", "label": "render", "file_type": "code", "source_file": "renderer.py"}],
        [{"source": "n1", "target": "n2", "relation": "references", "confidence": "INFERRED", "weight": 1.0}],
    )
...
def test_god_node_article_community_without_node_attr(tmp_path):
    ...
    G = graph_from_payload(
        [{"id": "n1", "label": "parse", "file_type": "code", "source_file": "parser.py"}, {"id": "n2", "label": "validate", "file_type": "code", "source_file": "parser.py"}],
        [{"source": "n1", "target": "n2", "relation": "calls", "confidence": "EXTRACTED", "weight": 1.0}],
    )
    communities = {0: ["n1", "n2"]}
    labels = {0: "Core Logic"}
...
def test_community_article_handles_null_source_file(tmp_path):
    ...
    G = graph_from_payload(
        [{"id": "n1", "label": "parse", "file_type": "code", "source_file": None, "community": 0}, {"id": "n2", "label": "validate", "file_type": "code", "source_file": "parser.py", "community": 0}],
        [{"source": "n1", "target": "n2", "relation": "calls", "confidence": "EXTRACTED", "weight": 1.0}],
    )
    communities = {0: ["n1", "n2"]}
...
def test_to_wiki_god_node_label_case_collides_with_community(tmp_path):
    ...
    G = _linked_pair(same_community=True)
    communities = {0: ["n1", "n2"]}
    labels = {0: "Parser"}
