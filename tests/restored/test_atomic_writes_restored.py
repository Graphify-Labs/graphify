"""Tests for generic atomic text/JSON writes and extraction manifests.
...
def test_write_text_atomic_writes_and_leaves_no_tmp(tmp_path):
    p = tmp_path / "out" / "payload.json"  # parent doesn't exist yet
    write_text_atomic(p, '{"a": 1}')
    assert json.loads(p.read_text()) == {"a": 1}
    # No leftover temp file in the target directory.
    assert [x.name for x in p.parent.iterdir()] == ["payload.json"]
...
def test_write_text_atomic_preserves_existing_on_failure(tmp_path, monkeypatch):
    ...
    assert sorted(x.name for x in tmp_path.iterdir()) == ["payload.json"]
...
def test_write_text_atomic_new_file_respects_umask(tmp_path):
    ...
    # 0600 — otherwise every fresh atomic output would be owner-only.
    p = tmp_path / "new.json"
    write_text_atomic(p, "{}")
    umask = os.umask(0)
    os.umask(umask)
...
def test_write_text_atomic_writes_through_symlink(tmp_path):
    # Shared-output setups may symlink an output to shared storage; the atomic write
