...
from graphify.export import to_cypher, to_graphml, to_html, to_canvas, to_obsidian
...
def make_graph():
    ...
    return graph_from_build(build_from_extraction(extraction))
...
def test_to_graphml_tolerates_none_attribute_values():
    ...
    G = graph_from_payload(
        [{"id": "a", "label": "A", "nullable_field": None}, {"id": "b", "label": "B"}],
        [{"source": "a", "target": "b", "nullable_field": None}],
    )
    communities = {0: ["a", "b"]}
    with tempfile.TemporaryDirectory() as tmp:
        out = Path(tmp) / "graph.graphml"
...
def test_to_graphml_tolerates_dict_and_list_attribute_values():
    ...
    G = graph_from_payload(
        [{"id": "a", "metadata": {"kind": "file", "size": 12}, "tags": ["x", "y"]}, {"id": "b"}],
        [{"source": "a", "target": "b", "ctx": {"k": "v"}}],
        graph={"hyperedges": [{"nodes": ["a"], "label": "h"}]},
    )
    communities = {0: ["a", "b"]}
    with tempfile.TemporaryDirectory() as tmp:
        out = Path(tmp) / "graph.graphml"
        ...
        data = {e.attrib["key"]: e.text for e in root.findall(".//{*}node/{*}data")}
        assert json.loads(data["metadata"]) == {"kind": "file", "size": 12}
...
def test_to_graphml_preserves_native_scalar_types():
    ...
    G = graph_from_payload(
        [{"id": "a", "count": 3, "ratio": 0.5, "flag": True, "name": "x"}, {"id": "b"}],
        [{"source": "a", "target": "b"}],
    )
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert json.loads(data["ratio"]) == 0.5
        assert json.loads(data["flag"]) is True
        assert data["name"] == "x"
...
def _two_node_graph():
    G = graph_from_payload(
        [
            {"id": "n1", "label": "Database", "community": 0, "source_file": "app/db.py", "type": "code"},
            {"id": "n2", "label": "Server", "community": 0, "source_file": "app/srv.py", "type": "code"},
        ],
        [{"source": "n1", "target": "n2"}],
    )
    return G, {0: ["n1", "n2"]}
...
def _four_node_two_community_graph():
    G = graph_from_payload(
        [
            {"id": "n1", "label": "Database", "community": 0, "source_file": "app/db.py", "type": "code"},
            {"id": "n2", "label": "Server", "community": 0, "source_file": "app/srv.py", "type": "code"},
            {"id": "n3", "label": "Cache", "community": 1, "source_file": "infra/cache.py", "type": "code"},
            {"id": "n4", "label": "Queue", "community": 1, "source_file": "infra/queue.py", "type": "code"},
        ],
...
def test_to_obsidian_case_only_distinct_labels_dont_overwrite():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert len(notes) == G.node_count, [p.name for p in notes]
...
def test_to_html_handles_null_source_file_and_label(tmp_path):
    """Null labels and source paths remain valid presentation-export values."""
    G = graph_from_payload([
        {"id": "n1", "label": "Foo", "source_file": None, "community": 0},
        {"id": "n2", "label": None, "source_file": "a.py", "community": 0},
        {"id": "n3", "label": None, "source_file": None, "community": 0},
    ])
