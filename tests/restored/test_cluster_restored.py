...
def make_graph():
    return graph_from_build(
        build_from_extraction(json.loads((FIXTURES / "extraction.json").read_text()))
    )
...
def test_cluster_does_not_write_to_stdout(capsys):
    ...
    Native Leiden diagnostics can emit ANSI escape sequences that break
