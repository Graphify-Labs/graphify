"""Tests for confidence_score on native edges."""
from graphify.build import build_from_extraction
from graphify.cluster import cluster, score_all
from graphify.analyze import god_nodes, surprising_connections
from graphify.report import generate
from graphify.helix.access import edge_rows
...
def _make_graph(extraction=None):
    return graph_from_build(build_from_extraction(extraction or _make_extraction()))


def test_extracted_edges_have_score_1():
    """EXTRACTED edges must have confidence_score == 1.0."""
    G = _make_graph()
    for u, v, d, _edge in edge_rows(G):
...
def test_confidence_score_round_trip():
    """confidence_score survives transient build → native generation round-trip."""
    G = _make_graph()
    links = [data for _u, _v, data, _edge in edge_rows(G)]
    assert links
    for link in links:
        assert "confidence_score" in link, f"Edge missing confidence_score: {link}"
...
def test_to_json_defaults_missing_confidence_score():
    """Edges lacking confidence_score get sensible native defaults."""
    extraction = {
        "nodes": [
    ...
    G = _make_graph(extraction)

    links_by_conf = {}
    for _u, _v, link, _edge in edge_rows(G):
