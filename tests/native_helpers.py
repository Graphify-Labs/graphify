...
def graph_from_payload(
    ...
    *,
    ...
    return make_loaded(nodes=nodes, edges=edges or [], kind=kind, graph=graph).graph


def graph_from_build(build: GraphBuildData):
    """Persist transient build data and return a real native snapshot."""
    return loaded_from_build(build).graph


def loaded_from_build(build: GraphBuildData):
    """Persist transient build data and return its graph plus native query handle."""
