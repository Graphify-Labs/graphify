"""Query seeding hygiene on a realistic corpus fixture (#2507).

Three behaviours share one fixture (`tests/seeding_fixtures.py`), because they
are three halves of the same natural-language question — "who calls X?":

1. **Relational-verb demotion** (landed in 0.9.35): a word naming the *relation*
   being asked about no longer buys an unconditional seed through the per-term
   guarantee (#1445). The tests in the first section are regression pins for
   that landed behaviour, re-stated against extraction-shaped wiring.
2. **Covered-term guarantee skip** (0.9.36): a term an already-picked seed's
   label plainly matches is not starved, so it claims no extra seed.
3. **Heuristic-filter starvation fallback** (0.9.36): an *inferred* context
   filter that discovers nothing beyond the seeds is relaxed and said so in the
   header; an *explicit* one is always honoured.

Assertions read external behaviour only — the `Start:` header, the rendered
NODE lines, the header's node count and `Context:` segment — never the
relational vocabulary itself, so tuning that vocabulary cannot break a test.
The one exception is the bound comparison in
`test_generic_noun_phrasing_seeds_no_hub_and_stays_bounded`, which reads the
vocabulary to *construct* its pre-fix baseline rather than to assert on it.
"""
...
def _context_segment(text: str) -> str:
...
# --------------------------------------------------------------------------- #
# Relational-verb demotion — regression pins for the 0.9.35 behaviour          #
#                                                                              #
# `_make_callers_graph` in test_serve.py pins the same change on a graph whose #
# call edges hang off the class node. These re-state it on the wiring          #
# extraction actually emits (calls land on the method; the class->method edge  #
# carries `context=None`) and against a decoy that sits in a busy test         #
# neighborhood, which is what made the seat expensive in the first place.      #
# --------------------------------------------------------------------------- #
...
def test_who_calls_phrasing_does_not_seed_verb_prefix_decoy():
    ...
    unconditional per-term seat. Seed level only — what the inferred `call`
    filter then does to the class seed is the starvation fallback's problem,
    pinned further down.
    """
...
def test_all_relational_query_keeps_its_per_term_guarantee():
    ...
    rather than decorative, and what makes it a sharper probe of the fallback
    than a single-word query is.
    """
    G = make_charge_fixture()
...
def test_scorer_and_picker_are_unchanged_for_direct_callers():
    """Demotion is wired from the query pipeline only. Callers that drive the
    scorer and the seed picker directly — `path`, `explain`, the
    legacy-equality property tests, the benchmark's two arms — must still see
    the relational term score and still receive its guaranteed seed."""
    G = make_charge_fixture()
...
# --------------------------------------------------------------------------- #
# Covered-term guarantee skip                                                  #
#                                                                              #
# A query term that an already-picked seed's normalized label plainly matches  #
# — the scorer's own weakest tier, so "this seed would have matched the term   #
# in scoring" — is not *starved*, and starvation is the only thing the #1445   #
# guarantee exists to prevent. It therefore claims no additional seed. The     #
# refined invariant: every term with any match is matched by at least one      #
# seed (previously: every term's own singleton winner gets a slot).            #
# --------------------------------------------------------------------------- #
...
# the repro measurements were taken at.
...
def test_generic_noun_phrasing_seeds_no_hub_and_stays_bounded():
    ...
    seed's label, so neither is starved and neither buys a seat. The bound is
    asserted against the pre-fix seed list's own traversal rather than a
    hard-coded number: the comparison mirrors 0.9.35's demotion so what it
    measures is this change alone. This phrasing triggers no context filter, so
    the comparison traversal is unfiltered like the pipeline's.
    """
    G = make_charge_fixture()
    ...
    demoted = {
        term: nid
        for term, nid in qs.best_seed_by_term.items()
        if term not in _RELATIONAL_INTENT_TERMS
    }
    pre_skip_seeds = _pick_seeds(qs.ranked, G=G, best_seed_by_term=demoted)
    pre_skip_nodes, _edges = _bfs(G, pre_skip_seeds, _GENERIC_NOUN_DEPTH)
    assert _nodes_found(text) < len(pre_skip_nodes), (
        f"traversal not bounded: {_nodes_found(text)} nodes from {seeds} is no smaller "
        f"than the {len(pre_skip_nodes)} the pre-skip seed list {pre_skip_seeds} reached"
    )
...
def test_covered_term_skips_guarantee_while_starved_term_is_still_recovered():
    ...
    guaranteed seat, and the genuinely starved term keeps its — the skip
    provably cannot reintroduce starvation.
    """
    G = make_charge_fixture()
    qs = _score_query(
...
# --------------------------------------------------------------------------- #
# Heuristic-context-filter starvation fallback                                 #
#                                                                              #
...
def test_who_calls_phrasing_falls_back_when_heuristic_filter_strands_the_seed():
    """"Who calls ChargeCustomerService?" end to end.
    ...
    reachable only through its `references` edge. The header keeps the failure
    mode visible: the heuristic context is still reported, annotated as relaxed,
    so a fallback answer never reads as a filtered one.
    """
    G = make_charge_fixture()
    text = _query_graph_text(G, "Who calls ChargeCustomerService?", mode="bfs", depth=2)
    seeds = start_labels(text)
    shown = shown_nodes(text)
    context = _context_segment(text)
...
def test_callers_of_phrasing_renders_all_callers_and_drops_junk_seed():
    """The agent-noun phrasing, "callers of X".

    0.9.35 added "caller"/"callers" to `_CONTEXT_HINTS`, so this phrasing now
    infers the same `call` filter "Who calls X?" does — and therefore inherits
    the same stranding on a class seed, where before the hint entry it
    traversed unfiltered. Same repair, same annotated header; the seed
    assertions additionally pin that the busy test method whose label carries
    "callers" stays out of both the seed list and the shown output.
    """
    G = make_charge_fixture()
    ...
    assert "relaxed" in _context_segment(text), (
        f"header does not report the relaxation: {_context_segment(text)!r}"
    )
...
def test_expanding_heuristic_filter_is_left_in_force():
    ...
    the `references`-only caller and the service class itself, whose `method`
    edge carries no context. Relaxing would pull both in.
    ...
    identifier and the assertions below read only the fallback's behavior.
    """
    G = make_charge_fixture()
...
def test_single_node_expansion_is_not_starvation():
    ...
    Same single-seed scenario as `test_expanding_heuristic_filter_is_left_in_force`,
    with one local tweak: the other caller's call edge into the service method is
    dropped, so the heuristic `call` filter reaches exactly one node beyond the
    seed instead of two. Pinning this boundary is what stops the threshold from
    drifting into a tuning constant (`<= len(seeds) + 1` and friends): such an
    implementation relaxes here — throwing away a filter that had in fact found
    its match, and dragging in the class node and the hub — which this test
    rejects. Asserted for both modes, since the threshold is shared.
    """
    for mode in ("bfs", "dfs"):
        G = make_charge_fixture()
        ...
        assert "relax" not in _context_segment(text), (
