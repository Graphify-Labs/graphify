...
def test_subgraph_to_text_seed_survives_truncation():
    ...
    text = _subgraph_to_text(
        G, nodes, list(graph.edges()), token_budget=30, seeds=["answer"]
    )
    ...
    G = make_loaded(nodes=[
        {
            "id": f"z{i}",
            "label": f"z{i}",
            "source_file": f"z{i}.py",
            "source_location": "L1",
            "community": 0,
        }
        for i in range(10)
    ])
    nodes = {node.id for node in _graph(G).nodes()}
...
def test_subgraph_to_text_ignores_dangling_src_tgt(monkeypatch):
    ...
    out = _subgraph_to_text(G, {"a", "b"}, _edge_records(G, ("a", "b")))
...
def test_subgraph_to_text_honors_valid_src_tgt_direction():
    """Native edge records retain the stored direction without sidecar fields."""
    G = make_loaded(
        nodes=[
            {
                "id": "caller",
    ...
    out = _subgraph_to_text(
        G,
        {"caller", "callee"},
        _edge_records(G, ("caller", "callee")),
    )
