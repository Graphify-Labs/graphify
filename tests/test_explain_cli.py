...
def test_explain_warns_on_ambiguous_match_and_resolves_case_exact(monkeypatch, tmp_path, capsys):
    """Two nodes whose labels differ only by case land in the same match tier.
    Upstream's find_node_ambiguity would refuse the query as ambiguous; the
    exact-case candidate settles it, so explain answers with that one instead
    of erroring."""
    graph_data = {
        "directed": False, "multigraph": False, "graph": {},
        "nodes": [
