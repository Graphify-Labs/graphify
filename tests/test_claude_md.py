...
def test_install_creates_settings_json(tmp_path):
    ...
    assert any(h.get("matcher") == "Bash|Grep|PowerShell" for h in hooks)
...
def test_install_settings_json_idempotent(tmp_path):
    ...
    settings = json.loads(settings_path.read_text())
    hooks = settings.get("hooks", {}).get("PreToolUse", [])
    bash_hooks = [h for h in hooks if h.get("matcher") == "Bash|Grep|PowerShell" and "graphify" in str(h)]
...
def test_uninstall_removes_settings_hook(tmp_path):
    ...
    if settings_path.exists():
        settings = json.loads(settings_path.read_text())
        hooks = settings.get("hooks", {}).get("PreToolUse", [])
        assert not any(h.get("matcher") == "Bash|Grep|PowerShell" and "graphify" in str(h) for h in hooks)
