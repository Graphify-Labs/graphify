"""Synthetic corpus fixture for the query seeding-hygiene work (#2507).

Shared by the three seeding changes the query pipeline now applies —
relational-intent demotion (landed in 0.9.35), the covered-term guarantee skip
and the heuristic-filter starvation fallback (both 0.9.36) — so all of them
grade the same three natural-language phrasings against one corpus instead of
re-inventing a graph each time.

The existing `_make_callers_graph` in test_serve.py attaches the call edges
straight to the class node, which is the one wiring real extraction never
produces; this fixture reproduces the measured shape instead, distilled from a
48.5k-node PHP repro graph:
...
* **three known callers**, two through real ``call``-context edges and one
  ...
  ``references`` edge, because its receiver is a docblock-typed,
  constructor-assigned property that PHP extraction does not resolve — an
  extraction-side gap deliberately reproduced rather than papered over.
...
* a **generic-noun hub class** (`Customer`) with a wide member fan-out, one
  reference hop away from the service — the traversal explosion the
  covered-term skip targets.
...
def make_charge_fixture(*, calls_symbol: bool = False) -> nx.Graph:
    ...
    # filter strands a class-node seed (the co-cause the starvation fallback
    # addresses).
    ...
    # property, so extraction never emits the `calls` edge. It stays reachable
    # through the import/reference edge it does get.
