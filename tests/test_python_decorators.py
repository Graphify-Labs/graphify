...
def test_pytest_mark_bare_is_not_a_decorator(tmp_path):
    # `mark` is a namespace root, not a decorator (pytest's MarkGenerator is
    # not callable): a bare `@mark` must keep its edge even when `mark` is
    # bound from pytest. Only a qualified `@mark.x` is pytest vocabulary.
    _write(tmp_path / "pkg" / "locals.py",
           "def mark():\n"
           "    return \"local helper\"\n")
    f = _write(tmp_path / "pkg" / "case.py",
               ...
               "    pass\n")
    r = extract([tmp_path / "pkg" / "locals.py", f], cache_root=tmp_path)
    assert _func_nid("pkg/locals.py", "mark") in _deco_edges(
        r, _func_nid("pkg/case.py", "fx")), (
        "bare @mark is not a pytest decorator and must keep its edge")


def test_pytest_mark_alias_qualified_is_suppressed(tmp_path):
    # `from pytest import mark as m`: the LOCAL alias `m` is bound to the mark
    # root, so a qualified `@m.x` is pytest vocabulary and must be suppressed.
    f = _write(tmp_path / "pkg" / "case.py",
    ...
    r = extract([f], cache_root=tmp_path)
    assert not _deco_edges(r, _func_nid("pkg/case.py", "test_x")), (
        "@m.parametrize must be suppressed as pytest vocabulary")
