...
def test_hop_order_holds_for_everything_the_answer_block_did_not_name():
    """The ordering contract, stated exactly.

    Nodes the ANSWER block named are pinned to the top whatever their distance —
    they are the answer, and they must survive the budget. Everything BELOW that
    is ordered hop-first (preserving #BUG2's intent that the seed keeps its
    immediate neighbourhood) with relevance deciding inside a layer.
    """
    G = nx.Graph()
    G.add_node("seed", label="alphaGate", source_file="src/alphaGate.ts",
               source_location="L1", community=0)
    # five strong hop-1 matches, enough to fill the answer block on their own
    for i in range(5):
    ...
    # an unmatched hop-1 node, and a weakly-matching hop-2 node behind it
    ...
    out = _query_graph_text(G, "alphaGate", depth=2, token_budget=4000)
    lines = [l for l in out.splitlines() if l.startswith("NODE ")]
    answer = out.split("ANSWER")[1].split("NODE ")[0]
    # `far` is too weak to be named in the answer, so it is ordered, not pinned
    assert "src/far.ts" not in answer
    near_at = next(i for i, l in enumerate(lines) if "src/near.ts" in l)
    far_at = next(i for i, l in enumerate(lines) if "src/far.ts" in l)
    assert near_at < far_at, "hop 1 must precede hop 2 among unpinned nodes"


def test_the_answer_block_nodes_are_pinned_above_the_hop_order():
    """The deliberate exception to hop ordering: a node the ANSWER block named is
    the answer, so it leads even when the traversal reached it late."""
    G = nx.Graph()
    ...
    lines = [l for l in out.splitlines() if l.startswith("NODE ")]
    assert "src/far.ts" in lines[1]    # named in the answer, so pinned
    assert "src/near.ts" in lines[2]   # matched nothing, so it follows
