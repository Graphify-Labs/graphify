...
the SKILL.md but never wrote `.gemini/agents/rules/graphify.md` or
`.gemini/agents/workflows/graphify.md` - even though the uninstall path removes them.
"""
...
def test_antigravity_project_install_writes_rules_and_workflows(tmp_path):
    ...
    skill = tmp_path / ".gemini/agents" / "skills" / "graphify" / "SKILL.md"
    rules = tmp_path / ".gemini/agents" / "rules" / "graphify.md"
    workflow = tmp_path / ".gemini/agents" / "workflows" / "graphify.md"
    assert skill.exists(), "skill should be installed under .gemini/agents/skills/"
...
def test_antigravity_project_uninstall_clears_rules_and_workflows(tmp_path):
    ...
    assert not (tmp_path / ".gemini/agents" / "rules" / "graphify.md").exists()
    assert not (tmp_path / ".gemini/agents" / "workflows" / "graphify.md").exists()
    assert not (tmp_path / ".gemini/agents" / "skills" / "graphify" / "SKILL.md").exists()
