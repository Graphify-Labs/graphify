...
@pytest.mark.parametrize("no_cluster", [True, False], ids=["no-cluster", "clustered"])
def test_rebuild_force_does_not_report_shrink_when_replace_fails(
    ...
    def fail_graph_replace(src, dst):
        replacements.append((src, dst))
        raise OSError("injected graph replacement failure")

    monkeypatch.setattr("graphify.watch.os_replace_with_fallback", fail_graph_replace)
    capsys.readouterr()
    source.write_text("def alpha():\n    return 1\n", encoding="utf-8")
    ...
    assert replacements == [(graph_path.parent / ".graph.tmp.json", graph_path)]
    assert graph_path.read_bytes() == before
    assert "--force: graph.json shrink: observed" not in capsys.readouterr().out
