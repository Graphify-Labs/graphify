...
| `GRAPHIFY_AGY_CLI_MODEL` | Antigravity CLI model | optional — default `gemini-3.8-flash-low`; also `--model` |
| `GRAPHIFY_AGY_CLI_PARALLEL` | Allow concurrent Antigravity CLI extraction and labeling calls | optional — serial by default; set `1` to enable |
| `GRAPHIFY_API_TIMEOUT` | Per-call timeout in seconds for HTTP, claude-cli, agy-cli, Anthropic SDK, and Bedrock backends (default: 600) | optional — also `--api-timeout` flag |
...
graphify extract ./docs --backend gemini       # explicit backend: gemini, kimi, claude, openai, deepseek, ollama, bedrock, claude-cli, or agy-cli
...
graphify extract ./docs --backend agy-cli      # route through Antigravity CLI - no API key, uses your Antigravity subscription
...
`agy-cli` requires an authenticated `agy` on `$PATH`. It defaults to
`gemini-3.8-flash-low`; override with `GRAPHIFY_AGY_CLI_MODEL` or `--model`.
Extraction and community labeling run serially unless `GRAPHIFY_AGY_CLI_PARALLEL=1`.
Prompts use one attached `-p=...` argument (stdin support is unverified), with
`--dangerously-skip-permissions` for headless operation. Extraction uses
`--json-schema`; images are supplied by path with `--add-dir`. Calls include
Antigravity's own system-prompt token overhead, charged to the subscription.
