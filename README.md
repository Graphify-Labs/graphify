...
| `acp` | Generic Agent Client Protocol provider (including Codex subscription through `codex-acp`; no Graphify API key) | install/configure an ACP adapter |
| `codex-cli` | Deprecated compatibility alias for `acp` | use `acp` for new configuration |
...
graphify extract ./docs --backend acp           # route through an ACP adapter, such as codex-acp
