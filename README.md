...
| OpenEdge ABL / Progress 4GL | `.p .w .i .cls` code + `.df` DB-schema dumps (AST; classes, procedures, functions, `runs`/`includes`/`uses`-table edges — `.df` tables/sequences become nodes the code links onto). Needs the [tree-sitter-abl](https://github.com/jgicquel/tree-sitter-abl) and [tree-sitter-df](https://github.com/usagi-coffee/tree-sitter-df) parsers installed via `uv tool install graphifyy --with <abl.whl> --with <df.whl>`. A content sniff routes ABL `.cls` files here and leaves Apex `.cls` on the Apex extractor. |
