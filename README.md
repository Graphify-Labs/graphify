...
| Salam | `.salam` (no extra dependency; English and Persian keywords, `switch`/`case`, Persian guillemet `«»` strings; packages, imports, functions, structs, methods, enums, interfaces, `impl`, extern C, layout/components, call and type-reference edges, `@en`/`@fa` name aliases) |
