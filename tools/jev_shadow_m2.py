...
from graphify.jev_shadow import JevShadowError, _nodes_edges, candidate_selection
...
def _reference(graph_path: Path, task: dict[str, Any]) -> tuple[set[str], set[str]]:
    """Inspectable pre-change structural reference, independent of Jev output."""
    graph = json.loads(graph_path.read_text(encoding="utf-8"))
    nodes, edges = _nodes_edges(graph)
    seeds = {node["id"] for node in nodes if node["source_file"] in set(task["changed_files"])} | set(task["seed_nodes"])
    # Direct call/import/containment/reference/test neighbours are the smallest
    # source-verifiable consumer/interface evidence available in the base graph.
    ...
    for edge in edges:
        if edge["id"] in reference_edges:
            reference_nodes.add(edge["source"]); reference_nodes.add(edge["target"])
    return reference_nodes, reference_edges
...
def evaluate(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        if not record or record.get("prepare_status") != "PREPARED":
            continue
        graph_path, task = Path(record["graph_path"]), record["task"]
        reference_nodes, reference_edges = _reference(graph_path, task)
        base = {"case_id": case["case_id"], "pr": case["pr"], "baseline": _baseline_details(record), "reference_node_count": len(reference_nodes), "reference_edge_count": len(reference_edges), "patch_categories": sorted({"test" if path.startswith("tests/") else "documentation" if path.endswith((".md", ".rst")) else "configuration" if path.endswith((".yml", ".yaml", ".toml", ".json")) else "code" for path in case["changed_files"]}), "runs": []}
        for strategy in STRATEGIES:
            for budget in BUDGETS:
                try:
                    ...
                    base["runs"].append({"strategy": strategy, "budget": budget, "status": "OK", "candidate_nodes": len(selected_nodes), "candidate_edges": len(selected_edges), "reference_node_recall": len(selected_nodes & reference_nodes) / len(reference_nodes) if reference_nodes else None, "reference_edge_recall": len(selected_edges & reference_edges) / len(reference_edges) if reference_edges else None, "mandatory_retained": set(result["metadata"]["mandatory_node_ids"]).issubset(selected_nodes), "source_file_coverage": len({node["source_file"] for node in result["nodes"] if node["source_file"]}), "test_evidence_retained": any(node["source_file"].startswith("tests/") for node in result["nodes"]), "consumer_evidence_retained": bool(selected_edges & reference_edges), "node_cap_reached": result["metadata"]["node_cap_reached"], "edge_cap_reached": result["metadata"]["edge_cap_reached"], "candidate_fingerprint": result["candidate_fingerprint"]})
