...
ALWAYS_ON_SANCTIONED_EDITS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    "_AGENTS_MD_SECTION": (
        ...
        # #2006: wiki/ is only ever regenerated by the explicit `export wiki`
        # subcommand, never by `update`/watch, so the unconditional "navigate
        # it instead of reading raw files" instruction can point agents at a
        # months-stale wiki with no signal anywhere in the loop. Point at the
        # new PreToolUse staleness nudge (cli.py) as the escape hatch instead.
        (
    ...
    # #2006: same fix as _AGENTS_MD_SECTION above, applied to antigravity's
