...
# prior run so Part C never merges a stale graphify-out/.graphify_cached.json (#1392).
...
After processing all files, write the accumulated result to `graphify-out/.graphify_semantic_new.json`.
...
"
rm -f graphify-out/.graphify_detect.json graphify-out/.graphify_extract.json graphify-out/.graphify_ast.json graphify-out/.graphify_semantic.json graphify-out/.graphify_analysis.json graphify-out/.graphify_labels.json; find graphify-out -maxdepth 1 -path 'graphify-out/.graphify_chunk_*.json' -delete 2>/dev/null
...
If `code_only` is False (any changed file is a doc/paper/image): run the full Steps 3A–3C pipeline as normal, then continue to the merge block below and Steps 4–8.
