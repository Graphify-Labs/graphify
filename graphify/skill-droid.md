...
> **graphify needs no API key. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all — a code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) uses Gemini through OpenRouter **only if** `GRAPHIFY_OPENROUTER_API_KEY`/`OPENROUTER_API_KEY` is already set; otherwise the host agent itself is the LLM. If you catch yourself about to prompt for, wait on, or stop because of a missing API key, that is a misread of this skill — proceed without one.

**Before semantic extraction:** check whether `GRAPHIFY_OPENROUTER_API_KEY` or `OPENROUTER_API_KEY` is set. If neither is set, print this one-liner to the user:
