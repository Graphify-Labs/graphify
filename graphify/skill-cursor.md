...
**Step B2 - Sequential extraction (Cursor)**

> **Cursor platform:** Cursor's parallel/background agents are separate sessions launched from the Cursor UI (or its own background-agents API) — they are not a tool you can call mid-conversation the way Claude Code's Agent tool or Codex's `spawn_agent` are. There is no in-chat primitive here for fanning work out to subagents, so extraction runs sequentially: you read and extract each file yourself. This is slower than the parallel platforms but fully reliable, and matches the fallback already used on Aider/OpenClaw for the same reason.

Print: `"Semantic extraction: N files (sequential — Cursor)"`

Process each file listed in `graphify-out/.graphify_uncached.txt` one at a time. For each file:
...
See `references/extraction-spec.md` for the exact schema (JSON schema, node-ID rules, confidence rubric, hyperedge, and vision rules). Load it only here, only when at least one file among the uncached set is a doc, paper, or image; a pure-code corpus has skipped Part B and never reads it.

3. Accumulate results across all files and write the combined result to `graphify-out/.graphify_semantic_new.json` once all files are processed.
