...
def _resolve_and_rank(
    ...
    *,
    ...
    scores the exact ranking `graphify query` serves.
    """
    ...
    if not start_nodes and semantic_scores:
        # Semantic rescue: a fuzzy question may share no token with any label, so
        # lexical seeding finds nothing — exactly when embeddings should take over.
