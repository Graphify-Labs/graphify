...
def find_node_ambiguity(G: nx.Graph, label: str) -> list[str]:
    ...
    for tier in _find_node_tiers(G, label):
        if not tier:
            continue
        # A single exact-case match settles the tie on its own. Tiers fold
        # case, so `GameInfo` and an unrelated `gameInfo` land together with
        # nothing but iteration order between them -- but when exactly one
