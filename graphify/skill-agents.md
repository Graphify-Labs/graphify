...
Merge all chunk files into `.graphify_semantic_new.json`. **After each Agent call completes, read real token usage from the tool result and write it back into that chunk's JSON before merging** — the chunk JSON template initializes tokens to `null` (unobserved).
