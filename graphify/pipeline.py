...
_SEMANTIC_REBUILD_EDGE_RELATIONS = {
    "calls",
    "implements",
    "references",
    "cites",
...
def _is_rebuilt_structural_edge(edge: dict, rebuilt_sources: set[str], root: Path) -> bool:
    ...
    if edge.get("relation") in _STRUCTURAL_REBUILD_EDGE_RELATIONS:
        return True
    # Upstream update treats edges as owned by the file that extracted them:
    # when that file is rebuilt, stale import/reference/inheritance edges from
    # the old extraction must disappear. Preserve semantic edges, which use the
    # same relation names but lack callsite/import-style provenance.
    if edge.get("relation") in _SEMANTIC_REBUILD_EDGE_RELATIONS:
        return bool(
            edge.get("context")
            or edge.get("contexts")
            or edge.get("source_location")
            or edge.get("callee_range")
            or edge.get("receiver_node_type")
        )
    return bool(edge.get("context") or edge.get("source_location"))
...
def merge_update_payload(
    ...
    *,
    ...
    preserved_nodes = [
        ...
        and not (full_rebuild and node.get("_origin") == "ast")
        and (
    ...
    hyperedge_evict_sources = rebuilt_source_set | evict_source_set
    preserved_hyperedges = []
    for hyperedge in existing.get("hyperedges", []):
        if not isinstance(hyperedge, dict):
            continue
        source = _source_key(hyperedge.get("source_file"), resolved_root)
        if source and source in hyperedge_evict_sources:
            continue
...
def merge_update_files(
    *,
    ...
    if incremental.get("files"):
        ...
        save_manifest(incremental["files"], root=resolved_root)
        manifest_saved = True
