...
Two more substitutions, in this step's block and Step 5's: replace `IS_NO_CLUSTER` with `True` if `--no-cluster` was given, otherwise `False`. Replace `IS_FORCE` with `True` if `--force` was given, otherwise `False`.
...
Before the merge step below, save the old graph so the post-update diff has something to compare against: `cp graphify-out/graph.json .graphify_old.json`
...
Clean up the backup after: `rm -f .graphify_old.json`
...
ANSWER
EOF
"$(cat graphify-out/.graphify_python)" -m graphify save-result --question "QUESTION" --answer-file graphify-out/.save_result_answer.tmp --type query --nodes NODE1 NODE2
```
