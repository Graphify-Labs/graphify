...
def _extract_python_rationale(path: Path, result: dict) -> None:
    ...
    def _scan_docstring_doc_refs(body_node) -> None:
        """Emit doc refs from a docstring, independent of the rationale length gate.

        `_get_docstring` drops anything under 20 characters because a one-word docstring
        is not rationale worth a node. A citation is not subject to that: a docstring
        reading only `Per ADR-11.` is under the gate and is exactly the edge this pass
        exists to create, so doc refs are read from the raw string node instead.
        """
        if not body_node:
            return
        for child in body_node.children:
                    ...
                    if sub.type in ("string", "concatenated_string"):
                        ...
                        for m in _DOC_REF_RE.finditer(text):
                            _emit_doc_ref(
                                ...
                                seen_doc_refs=seen_doc_refs, file_nid=file_nid,
                                str_path=str_path,
                            )
            break
