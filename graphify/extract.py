...
SEMANTIC_RELATIONS = frozenset({
    ...
    "calls", "imports", "imports_from", "re_exports", "contains", "method","satisfies",
})
...
# Requirement IDs like SAFE-04, DRIVE-05, AUTH-10
_REQUIREMENT_ID_RE = re.compile(
    r"\b([A-Z][A-Z0-9_]*-\d+)\b"
)
...
def _extract_requirement_ids(text: str) -> list[str]:
    """
    Extract requirement identifiers from text.
    ...
    return [
        match.group(1)
        for match in _REQUIREMENT_ID_RE.finditer(text)
    ]
def _extract_python_requirements(path: Path, result: dict) -> None:
    """
    Post-pass: extract requirement IDs from Python docstrings/comments.

    Creates:
        code_symbol ---- satisfies ----> requirement
    ...
    def _add_requirement(req_id, line, symbol_id):

        req_node_id = _make_id("requirement", req_id)

        if req_node_id not in seen_ids:
            seen_ids.add(req_node_id)
    ...
    def _scan_text(text, line, symbol_id):
    ...
    for lineno, line in enumerate(
        ...
        if "#" in line:
            ...
            req_ids = _extract_requirement_ids(comment)
...
def extract_python(path: Path) -> dict:
    ...
    if "error" not in result:
        ...
        _extract_python_requirements(path, result)
    return result
