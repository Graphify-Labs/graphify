...
def _load_existing_graph(graph_path: Path) -> "tuple[list, list, list, bool] | None":
    """Load (nodes, edges, hyperedges, multigraph) from an existing graph.json
    for an incremental merge, accepting both the ``links`` and ``edges``
    spellings. The stored ``multigraph`` flag comes back from the same read so
    callers can infer the format without parsing the file twice.
...
def build_merge(
    ...
    *,
    ...
    if _loaded is not None:
        existing_nodes, existing_edges, existing_hyperedges, _existing_multigraph = _loaded
        had_graph = True
