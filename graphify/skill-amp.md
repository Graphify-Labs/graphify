...
> **graphify needs no API key. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all — a code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) uses Gemini through OpenRouter when `GRAPHIFY_OPENROUTER_API_KEY` is already set, or the legacy direct Gemini route when `GEMINI_API_KEY`/`GOOGLE_API_KEY` is already set; otherwise the host agent itself is the LLM. If you catch yourself about to prompt for, wait on, or stop because of a missing API key, that is a misread of this skill — proceed without one.

**Before semantic extraction:** check `GRAPHIFY_OPENROUTER_API_KEY` first, then the legacy `GEMINI_API_KEY`/`GOOGLE_API_KEY`. If none is set, print this one-liner to the user:
...
Print it once, then continue — do not wait for the user to supply a key. If any supported Gemini credential IS set, use `graphify.llm.extract_corpus_parallel(files, backend="gemini")` for semantic extraction instead of dispatching subagents. The `gemini` backend selects the Gemini request shape and model; runtime credential precedence chooses the dedicated OpenRouter route first, then the legacy direct route. The exact default model is `gemini-3-flash-preview` (sent as `google/gemini-3-flash-preview` through OpenRouter); set `GRAPHIFY_GEMINI_MODEL` or pass `--model` to override it without substitution.

> **No other API keys are read by this skill.** When all supported Gemini credentials are unset, semantic extraction falls to the host agent itself — the running session is the LLM. On a host that dispatches subagents (e.g. Claude Code), dispatch them as written in Part B. On a host that runs the CLI directly and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, use an already-configured supported Gemini credential or extract those inline yourself, but never prompt for another provider key.
