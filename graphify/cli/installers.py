"""graphify installer commands.

Extracted from graphify/__main__.py in the Phase 5 refactor. Owns the
installation / uninstallation / version-stamp helpers for every
supported AI platform (Claude Code, Gemini CLI, VSCode, Kiro,
Antigravity, Cursor, Devin, Kilo, OpenCode, Codex, Amp, CodeBuddy),
plus the cross-platform helpers (_always_on, _skill_registration,
_platform_skill_destination, etc.) and the _clone_repo utility used by
``graphify clone <github-url>``.

The public install/uninstall entry points are re-exported from
``graphify.__main__`` so existing imports like
``from graphify.__main__ import claude_install`` keep working.
"""
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    ...
    if platform == "kilo":
        ...
        command_src = Path(__file__).parent.parent / "command-kilo.md"
