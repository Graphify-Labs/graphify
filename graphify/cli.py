...
def _read_nudge_stale(out_name: str) -> str:
    return json.dumps({
        "hookSpecificOutput": {
            "hookEventName": "PreToolUse",
            "additionalContext": (
                f'{out_name}/graph.json exists but may be STALE for this file (the file '
...
def _read_nudge_wiki_stale(out_name: str) -> str:
    return json.dumps({
        "hookSpecificOutput": {
            "hookEventName": "PreToolUse",
            "additionalContext": (
                f'{out_name}/wiki/ predates the current graph.json. Unlike graph.json, '
    ...
    try:
        if kind == "search":
                ...
                if "/wiki/" in j:
                    try:
                        ...
                        if wiki_mtime < gmtime:
                            sys.stdout.write(_read_nudge_wiki_stale(GRAPHIFY_OUT_NAME))
                    except OSError:
                        pass
                return
            ...
            try:
                ...
                pass
            if stale:
                sys.stdout.write(_read_nudge_stale(GRAPHIFY_OUT_NAME))
                return
