...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --one-file-system       don't cross mount points (find -xdev); auto-on for a / or $HOME root")
        ...
        print("  serve-html              serve graphify-out/ over HTTP and print the graph.html URL (headless hosts)")
        print("    --host H                interface to bind (default 127.0.0.1; --host 0.0.0.0 exposes it on the network)")
        ...
        print("    --one-file-system       don't cross mount points (find -xdev); auto-on for a / or $HOME root")
        print("    --exclude PATTERN       gitignore-style exclude, anchored at the scan root (repeatable)")
        print("    --no-cluster            skip clustering, write raw extraction only")
