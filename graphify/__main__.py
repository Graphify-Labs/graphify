...
# Source of the Claude progressive-disclosure split tree (generated by
# graphify/skill_build.py from skill.md): a lean SKILL.md + reference/*.md.
_SPLIT_SKILL_DIR = Path(__file__).parent / "skill_claude"
...
def _copy_split_skill(skill_dst: Path) -> Path:
    """Install the progressive-disclosure skill tree (SKILL.md + reference/).

    Reference files are written first and SKILL.md last, so a crash mid-install
    never leaves a complete SKILL.md pointing at missing reference files.
    """
    if not (_SPLIT_SKILL_DIR / "SKILL.md").exists():
        print("error: skill_claude/ not found in package - reinstall graphify", file=sys.stderr)
        sys.exit(1)
    skill_dir = skill_dst.parent
    ref_dst = skill_dir / "reference"
    ref_dst.mkdir(parents=True, exist_ok=True)
    for f in sorted((_SPLIT_SKILL_DIR / "reference").glob("*.md")):
        _atomic_copy(f, ref_dst / f.name)
    _atomic_copy(_SPLIT_SKILL_DIR / "SKILL.md", skill_dst)  # last, for crash-safety
    (skill_dir / ".graphify_version").write_text(__version__, encoding="utf-8")
    print(f"  skill installed  ->  {skill_dst} (+ reference/)")
    return skill_dst


def _copy_skill_file(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    """Copy a packaged skill file (or split tree) and write its version stamp."""
    skill_dst = _platform_skill_destination(platform_name, project=project, project_dir=project_dir)
    if _PLATFORM_CONFIG.get(platform_name, {}).get("split"):
        skill_dst.parent.mkdir(parents=True, exist_ok=True)
        return _copy_split_skill(skill_dst)
    ...
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    _atomic_copy(skill_src, skill_dst)
    (skill_dst.parent / ".graphify_version").write_text(__version__, encoding="utf-8")
    print(f"  skill installed  ->  {skill_dst}")
    return skill_dst


def _remove_skill_tree(skill_dst: Path, *, announce: bool = True) -> bool:
    """Remove an installed skill: SKILL.md, its reference/ dir, the version stamp.

    Handles both the single-file (flatten) layout and the split layout. Removes
    only graphify-owned names — never a blind rmtree of the skill dir — then
    prunes now-empty parent dirs. Returns True if anything was removed.
    """
    skill_dir = skill_dst.parent
    removed = False
    if skill_dst.exists():
...
def _remove_skill_file(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> bool:
    """Remove a platform skill (file or split tree) without touching other scopes."""
    skill_dst = _platform_skill_destination(platform_name, project=project, project_dir=project_dir)
    return _remove_skill_tree(skill_dst)
...
def _kiro_uninstall(project_dir: Path) -> None:
    ...
    _remove_skill_tree(skill_dst, announce=False)
...
def _antigravity_uninstall(project_dir: Path, *, project: bool = False) -> None:
    ...
    # Remove skill file (+ reference/ if a split tree was installed)
