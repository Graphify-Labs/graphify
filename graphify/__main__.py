...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --source-root <path>    confirm merged C/C++ calls with local Clang")
        ...
        print("    --max-nodes N           cap semantic traversal nodes (default 80, max 200)")
        print("    --format F              text or evidence-json (default text)")
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
