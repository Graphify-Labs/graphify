...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("  path \"A\" \"B\"            shortest path between two nodes in graph.helix")
        print("    --store <path>          Helix store (default graphify-out/graph.helix)")
        ...
        print("    --retain-rollback      retain one previous native generation")
        print("  rollback [--store PATH] activate the explicitly retained previous generation")
        print("  doctor [--deep]         verify native counts or perform a full checksum audit")
        print("    --force                 force a full source rescan")
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("  cluster-only <path>     rerun clustering on an existing graph.helix generation")
        print("    --no-viz                skip graph.html generation (useful for >5000 node graphs / CI)")
        print("    --store <path>          Helix store (default <path>/graphify-out/graph.helix)")
        ...
        print("    --batch-size=N          communities per labeling LLM call (default 100)")
        print("  query \"<question>\"       native traversal of graph.helix for a question")
        ...
        print("    --store PATH            graph.helix store for community grouping (optional)")
        ...
        print("  tree                    emit a D3 v7 collapsible-tree HTML from graph.helix")
        print("    --store PATH            Helix store (default graphify-out/graph.helix)")
        ...
        print("  global add <project|graph.helix>  add/update a project in ~/.graphify/global-graph.helix")
        print("    --retain-rollback      retain one previous global generation")
