...
The platform-specific installer code (Claude, Gemini, VSCode, Kiro,
Antigravity, Cursor, Devin, Kilo, OpenCode, Codex, Amp, CodeBuddy) and
the cross-platform helpers used to live in this module. They have been
moved to ``graphify.cli.installers`` in the Phase 5 refactor; the
public names are re-exported below so existing
``from graphify.__main__ import claude_install`` imports keep working.
"""
...
from graphify.cli import installers as _installers
# Public install entry points — imported explicitly so ruff / mypy / pyright
# can see them as module-level definitions and the ``__all__`` below stays
# valid. ``main()`` and the other subcommand dispatchers reach for these
# directly when they don't go through the re-export binder below.
from graphify.cli.installers import (
    install,
    uninstall_all,
    claude_install,
    claude_uninstall,
    ...
    vscode_install,
    vscode_uninstall,
    # Cross-platform helpers the ``main()`` dispatcher also references by
    # name (e.g. when computing skill-destination paths during the
    # version-stamp check). Without these, ruff reports F821 even though
    # the re-export binder below makes them resolvable at runtime.
    _GRAPHIFY_OUT,
    __version__,
    _PLATFORM_CONFIG,
...
# Re-export every installer name as a *module attribute* on
# ``graphify.__main__`` (not just a global in this file) so external
# imports like ``from graphify.__main__ import _CLAUDE_MD_SECTION`` keep
# working — the re-exports used to be top-level defs in the monolithic
# __main__ module, and tests in test_install_strings / test_claude_md /
# test_install_references / test_install_roundtrip all reach for these
# private names directly. This preserves the pre-refactor public-ish
# surface without forcing every test to learn the new location.
def _bind_installer_names_to_module() -> None:
    target = _sys.modules[__name__]
    for name in dir(_installers):
        if not name.startswith("__") or name == "__version__":
            setattr(target, name, getattr(_installers, name))
    # ``_ALWAYS_ON_ALIASES`` are PEP 562 lazy attributes on
    # ``cli.installers`` — they only resolve via ``__getattr__`` and
    # therefore don't show up in ``dir()``. Tests like
    # test_install_strings import these by name directly off
    # ``graphify.__main__``, so bind them eagerly here.
    for alias_name in _installers._ALWAYS_ON_ALIASES:
        setattr(target, alias_name, getattr(_installers, alias_name))


_bind_installer_names_to_module()
