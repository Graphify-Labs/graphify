...
def main() -> None:
    ...
    if cmd == "install":
        ...
        # Merge AST + semantic + optional introspection, then apply
        # configured post-AST enrichments.
        merged = _merge_ast_semantic(ast_result, sem_result)
        for extra_result in (pg_result, cargo_result):
            if any(extra_result.get(key) for key in ("nodes", "edges", "hyperedges")):
                merged["nodes"].extend(extra_result.get("nodes", []))
                merged["edges"].extend(extra_result.get("edges", []))
                merged["hyperedges"].extend(extra_result.get("hyperedges", []))
                merged["input_tokens"] += extra_result.get("input_tokens", 0)
                merged["output_tokens"] += extra_result.get("output_tokens", 0)
        try:
