...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("  orcarouter <cmd>        OrcaRouter provider: login (API key or OAuth 2.0 + PKCE), status, models")
        print("    login [--flow loopback|oob]  sign in with OrcaRouter, or store an existing key (--api-key)")
        print("    key set|clear           store or remove an sk-orca-… API key")
        print("    status                  show the active credential (masked) and store location")
        print("    models [--capability C] list the live catalog (chat|vision|embedding|image|video|rerank)")
        print("    use <model-id>          pin a default model for this provider")
