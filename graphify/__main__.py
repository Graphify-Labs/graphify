...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
        print("  stale                   list nodes outside files changed since the last extraction")
        print("                          that reference those files — worth re-checking, not confirmed stale")
        print("    --relation R            edge relation to traverse in reverse (repeatable)")
        print("    --depth N               reverse traversal depth (default 1)")
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
