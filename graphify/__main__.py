...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("    --skip-semantic         skip LLM extraction completely, retaining only AST nodes and video transcripts")
    ...
    if cmd == "install":
        ...
        if len(sys.argv) < 3:
            print(
                ...
                "[--skip-semantic] [--max-workers N] [--token-budget N] [--max-concurrency N] "
                "[--api-timeout S]",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        while i < len(args):
            ...
            if a == "--backend" and i + 1 < len(args):
                ...
                dedup_llm = True; i += 1
            elif a == "--skip-semantic":
                skip_semantic = True; i += 1
        ...
        if video_files:
            ...
            transcript_paths = _transcribe_all([str(p) for p in video_files])
            doc_files.extend(Path(p) for p in transcript_paths)
        ...
        if skip_semantic:
            ...
            image_files = []
            dedup_llm = False
            print("[graphify extract] skipping semantic extraction (--skip-semantic)")

        semantic_files = doc_files + paper_files + image_files
        if incremental_mode:
            print(
                f"[graphify extract] {len(code_files)} code, {len(doc_files)} docs, "
                f"{len(paper_files)} papers, {len(image_files)} images, {len(video_files)} videos changed; "
