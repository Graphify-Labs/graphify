...
Cross-file concept IDs (NON-CODE nodes): the `{stem}_{entity}` rule above exists to match the AST extractor for **code symbols**. But named concepts, people, products, and rationale nodes (`file_type` of `concept`, `rationale`, `document`, or `paper`) routinely recur across many files and chunks — and a doc-only or dialogue corpus has no AST id to match. For those nodes, derive the ID from the ENTITY ALONE, **not** the file: a stable canonical slug of the concept's name (lowercased, `[a-z0-9_]`), preferably typed — `person_nikita`, `product_myown_academy`, `concept_dom_first`, `org_hetzner`. The same concept mentioned in three different files MUST produce the same ID so the chunks merge into one well-connected node instead of three orphans. Do NOT prefix non-code concept IDs with the file stem — file-stem prefixing is the single biggest cause of empty god-node rankings and zero cross-file edges on doc/paper/dialogue corpora (the graph fragments into one tiny component per chunk).
