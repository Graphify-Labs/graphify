...
Then write the JSON to disk using an available file-writing tool at this exact absolute path (no relative paths — file-writing tools may resolve relative paths against an undefined cwd and the file will be silently lost):
CHUNK_PATH
```
