...
- Relations are closed vocabularies. Edge relation MUST be exactly one of `calls`, `implements`, `references`, `cites`, `conceptually_related_to`, `shares_data_with`, `semantically_similar_to`, `rationale_for`. Hyperedge relation MUST be exactly one of `participate_in`, `implement`, `form`. Never invent or alter a label.
...
source_location RULE: every node, edge, and hyperedge must cite the smallest exact supporting span. UTF-8 text uses `L<line>` or inclusive `L<start>-L<end>`. PDF/raster evidence uses half-open `B<start>-B<end>` and may cite the whole-file byte span in `graphify-out/.graphify_source_manifest.json` when no finer mapping exists. Graphify validates the current path, bounds, and snapshotted digest and stamps `source_sha256`; do not emit the digest yourself. Missing, null, malformed, stale, or non-resolving provenance is rejected before merge or persistence.

Write the JSON to the exact absolute path CHUNK_PATH. After the write succeeds, return only a completion status. Do not repeat the JSON inline; CHUNK_PATH is the sole accepted semantic result.
```
