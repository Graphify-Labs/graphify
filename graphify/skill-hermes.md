...
**MANDATORY: You MUST use `delegate_task` here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not delegate you are doing this wrong.**
...
**Step B2 - Dispatch ALL subagents in ONE `delegate_task` call**

Hermes batches children through a single call: `delegate_task(tasks=[{...}, {...}, ...])`
runs them in parallel, capped by `delegation.max_concurrent_children`. One task
entry per chunk. Making several separate `delegate_task` calls serialises the work
and defeats the purpose.

**IMPORTANT - each child is isolated:** a subagent knows nothing about this
conversation, so every task entry must carry its own full `context` (the file
list, the absolute CHUNK_PATH to write, and the extraction prompt). Children have
file and terminal access and can write their chunk file; they cannot ask you
questions mid-run.

Concrete example for 3 chunks (ONE call):
```
delegate_task(tasks=[
  {goal: "Extract entities from files 1-15",  context: "<prompt + FILE_LIST + CHUNK_PATH .../.graphify_chunk_01.json>"},
  {goal: "Extract entities from files 16-30", context: "<prompt + FILE_LIST + CHUNK_PATH .../.graphify_chunk_02.json>"},
  {goal: "Extract entities from files 31-45", context: "<prompt + FILE_LIST + CHUNK_PATH .../.graphify_chunk_03.json>"},
])
```
...
The `delegate_task` batch returns one consolidated message when every child is done. For each result:
- Check that `graphify-out/.graphify_chunk_NN.json` exists on disk — this is the success signal
- If the file exists and contains valid JSON with `nodes` and `edges`, include it and save to cache
- If the file is missing, the subagent finished without writing — print a warning: "chunk N missing from disk". Its final summary is a self-report, so never treat "done" as proof; the file on disk is the only success signal. Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run; check `cache/delegation/live/<delegation_id>/task-N.log` for what the children actually did.

Merge all chunk files into `.graphify_semantic_new.json`. **If the delegation result reports token usage, write the real counts back into each chunk JSON before merging** — the chunk JSON itself always has placeholder zeros. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
