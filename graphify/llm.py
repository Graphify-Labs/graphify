...
BACKENDS: dict[str, dict] = {
    ...
    "kimi": {
        ...
        # valid_efforts ["low","high","max"] on /models). Upstream previously sent
        # nothing, so the server default ("high" for K3) silently applied while the
        # gemini block above has an effort setting. Env-overridable; forwarded as
        # `reasoning_effort` by the existing plumbing.
