...
def _extract_generic(
    ...
    def walk_calls(
        node,
        caller_nid: str,
        # Java values are plain short names; C# is the (scoped bindings,
        # field base) pair from _csharp_method_receiver_types, resolved
        # positionally (#2472); PHP values are a (short, qualified) pair and
        # may be None — see _php_method_receiver_types. Body ids are
        # language-disjoint, so one body's table only ever meets its own
        # language's reader.
        receiver_types: dict[str, str | _PhpReceiverType | None] | tuple | None = None,
    ...
    # Body ids are unique (one language per file), so the Java (flat), PHP
    # (flat) and C# (scoped, #2472) per-method receiver tables merge without
    # collision — the stamp site branches on language to read the matching
    # shape.
