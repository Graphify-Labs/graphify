...
def _extract_generic(
    ...
    if config.ts_module == "tree_sitter_c_sharp":
        csharp_interface_names = _csharp_extract._csharp_pre_scan_interfaces(root, source)
    ...
    def walk(node, parent_class_nid: str | None = None) -> None:
        ...
        if t in config.class_types:
            ...
            metadata = None
            if config.ts_module == "tree_sitter_c_sharp":
                metadata = _csharp_extract.csharp_class_member_metadata(
                    node, source, parent_class_nid
                )
            ...
            if config.ts_module == "tree_sitter_c_sharp":
                csharp_type_params = _csharp_extract._csharp_type_parameters_in_scope(node, source)
                for base, qualified, qualifier, relation, generic_refs in (
                    _csharp_extract.csharp_base_list_facts(
        ...
        if (config.ts_module == "tree_sitter_c_sharp"
                ...
                and parent_class_nid):
            line = node.start_point[0] + 1
            for type_name, context, qualified, qualifier in (
                _csharp_extract.csharp_field_type_ref_facts(node, source)
            ):
        ...
        if (config.ts_module == "tree_sitter_c_sharp"
            ...
            line = node.start_point[0] + 1
            for ref_name, context, qualified, qualifier in (
                _csharp_extract.csharp_property_type_ref_facts(node, source)
            ):
                target_nid = ensure_named_node(ref_name, line)
                if target_nid != parent_class_nid:
                    metadata = {"ref_token": ref_name}
        ...
        if t in config.function_types:
            ...
            if config.ts_module == "tree_sitter_c_sharp" and t == "method_declaration":
                func_metadata = {
                    "csharp_return_type": _csharp_extract._csharp_declared_bare_type(
            ...
            if config.ts_module == "tree_sitter_c_sharp":
                for ref_name, context, qualified, qualifier in (
                    _csharp_extract.csharp_method_reference_facts(node, source)
                ):
                    target_nid = ensure_named_node(ref_name, line)
                    if target_nid != func_nid:
                        metadata = {"ref_token": ref_name}
        ...
        if config.ts_module == "tree_sitter_c_sharp":
            if _csharp_extract._csharp_extra_walk(
                node, source, file_nid, stem, str_path,
                nodes, edges, seen_ids, function_bodies,
                parent_class_nid, add_node, add_edge, walk,
    ...
    def walk_calls(
        ...
        if node.type in config.call_types:
            ...
            if config.ts_module == "tree_sitter_swift":
                ...
                callee_name, is_member_call, member_receiver = (
                    _csharp_extract.csharp_invocation_callee(node, source)
                )
            ...
            if callee_name and callee_name not in _LANGUAGE_BUILTIN_GLOBALS:
                ...
                # Java uses the same defer-first shape so receiver types can be
                # resolved by the cross-file Java pass instead of by bare label.
                _csharp_defer = (
                    config.ts_module == "tree_sitter_c_sharp"
                    and is_member_call
                    and member_receiver is not None
                )
                _java_defer = config.ts_module == "tree_sitter_java" and is_member_call
                if _csharp_defer or _java_defer or (
                    is_member_call
                    and member_receiver
                    and (
                ...
                if tgt_nid and tgt_nid != caller_nid:
                    ...
                    if config.ts_module == "tree_sitter_c_sharp":
                        ...
                        rc_entry["scope_chain"] = _csharp_extract._csharp_scope_chain(node)
    ...
    if config.ts_module == "tree_sitter_c_sharp":
        result.update(_csharp_extract.csharp_file_facts(root, source, str_path))
    return result
