...
# (`from pytest import fixture`): the import-scope scan suppresses only those,
# not every name pytest happens to export (`raises`, `approx`, …).
_PYTEST_DECORATOR_EXPORTS = frozenset({"fixture", "hookimpl", "hookspec"})
# `mark` is a separate bare root (`from pytest import mark`): `@mark.x` uses
# the open-ended `pytest.mark.` prefix, not a `pytest.mark` decorator itself.
_PYTEST_MARK_EXPORTS = frozenset({"mark"})

@dataclass(frozen=True)
class _PythonBindingEvent:
    """One module-level pytest binding, resolved in source order.

    kind is "pytest_module" (a name bound to the pytest module itself:
    `import pytest`, `import pytest as pt`), "pytest_symbol" (a decorator
    rebound from pytest's namespace: `from pytest import fixture`),
    "pytest_mark" (the `mark` root, which is only a decorator when qualified:
    `from pytest import mark`, `@mark.x`), "rebound"
...
def _extract_generic(
    ...
    if config.ts_module == "tree_sitter_python":
        pytest_bindings = _python_module_pytest_bindings(root, source)
            ...
            if config.ts_module == "tree_sitter_python":
                ...
                if inner_name and normalize_id(inner_name):
                    ...
                    for child in node.children:
                        ...
                        if _python_is_pytest_decorator(
...
def _python_module_pytest_bindings(root, source: bytes) -> list[_PythonBindingEvent]:
    """Module-level pytest binding events, in source order (see `_PythonBindingEvent`).

    `_python_is_pytest_decorator` replays the events up to a decorator's byte
    ...
    descended into: this scanner tracks module-level bindings only, matching
    the scope boundary used by `_python_module_bound_names`. This is a
    conservative tracker for pytest decorator classification only, not a
    complete Python binding model: a wildcard `from pytest import *` records
    no events, so it is conservatively not treated as pytest vocabulary.
    """
                ...
                for c in child.children:
                    if c.type == "dotted_name":
                        if _read_text(c, source) == "pytest":
                            events.append(_PythonBindingEvent(
                                c.start_byte, "pytest_module", "pytest"))
                        ...
                        if name_node is not None and _read_text(name_node, source) == "pytest":
                            ...
                            events.append(_PythonBindingEvent(c.start_byte, "pytest_module", alias))
                    ...
                    if level == 0 and module_name == "pytest":
                        for name, local_name in _python_imported_names(child, source):
                            if name in _PYTEST_DECORATOR_EXPORTS:
                                events.append(_PythonBindingEvent(
                                    child.start_byte, "pytest_symbol", local_name))
                            elif name in _PYTEST_MARK_EXPORTS:
                                events.append(_PythonBindingEvent(
                                    child.start_byte, "pytest_mark", local_name))
...
def _python_binding_at(
    bindings: list[_PythonBindingEvent],
    name: str,
    byte_pos: int,
) -> str | None:
    """Last binding kind for `name` at or before `byte_pos`, or None."""
...
def _python_is_pytest_decorator(
    deco_path: str | None,
    pytest_bindings: list[_PythonBindingEvent],
    byte_pos: int,
) -> bool:
    ...
    if not deco_path:
        return False
    root_name = deco_path.partition(".")[0]
    binding = _python_binding_at(pytest_bindings, root_name, byte_pos)
    if binding == "pytest_symbol":
        return True
    if binding == "pytest_mark":
        # `mark` is a namespace root, not a decorator (MarkGenerator is not
        # callable): only a qualified use (`@mark.x`, `@mark as m` -> `@m.x`)
        # is pytest vocabulary, never a bare `@mark`.
        suffix = deco_path[len(root_name):]
        if not suffix:
            return False
        pytest_path = f"pytest.mark{suffix}"
        return pytest_path.startswith("pytest.mark.")
    if binding != "pytest_module":
        return False
    suffix = deco_path[len(root_name):]
    pytest_path = f"pytest{suffix}"
    if pytest_path in _PYTEST_DECORATOR_PATHS:
        return True
    return any(pytest_path.startswith(prefix) for prefix in _PYTEST_DECORATOR_PREFIXES)
