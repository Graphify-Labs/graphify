"""Apex extractor for Salesforce .cls and .trigger files."""
...
def extract_apex(path: Path) -> dict:
    ...
    annotation_re = re.compile(r"@(\w+)", re.IGNORECASE)
    soql_re = re.compile(r"\[\s*SELECT\b[^\]]+FROM\s+(\w+)", re.IGNORECASE)
    dml_re = re.compile(r"\b(insert|update|delete|upsert|merge|undelete)\s+\w", re.IGNORECASE)
    ...
    for lineno, line_text in enumerate(lines, start=1):
        ...
        if im:
            ...
            if iface_name.lower() in _CONTROL_FLOW:
                pending_annotations = []
                continue
            iface_nid = make_id(stem, iface_name)
        ...
        if em:
            ...
            if enum_name.lower() in _CONTROL_FLOW:
                pending_annotations = []
                continue
            enum_nid = make_id(stem, enum_name)
        ...
        if current_class_nid is not None:
            ...
            if mm:
                ...
                if method_name.lower() not in _CONTROL_FLOW:
                    method_nid = make_id(current_class_nid, method_name)
