"""SQL extractor - extracts tables, views, functions, and relationships."""
...
def extract_sql(path: Path, content: str | bytes | None = None) -> dict:
    ...
    def walk(node) -> None:
        ...
        if t == "create_table":
                            ...
                            if cd.type == "column_definition":
                                ...
                                for cc in cd.children:
                                    if cc.type == "keyword_references":
                                        ...
                                        break
                                if ref_name:
                                    ref_nid = table_nids.get(ref_name.lower()) or make_id(
                                        stem, ref_name
                                    )
            ...
            if trig_name:
                ...
                if tbl_name:
                    tbl_nid = table_nids.get(tbl_name.lower()) or make_id(stem, tbl_name)
            ...
            if m:
                ...
                if obj_type == "TRIGGER":
                    ...
                    if fm:
                        tbl = fm.group(1)
                        tbl_nid = table_nids.get(tbl.lower()) or make_id(stem, tbl)
    ...
    for m in re.finditer(r"CREATE\s+TABLE\s+([\w$]+)\s*\(", src_text, re.IGNORECASE):
        ...
        for rm in re.finditer(r"\bREFERENCES\s+([\w$]+)", block, re.IGNORECASE):
            ref_name = rm.group(1)
            ref_nid = table_nids.get(ref_name.lower()) or make_id(stem, ref_name)
