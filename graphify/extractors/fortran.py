"""Fortran extractor - custom walk for Fortran files."""
...
def extract_fortran(path: Path) -> dict:
    ...
    try:
        ...
        source = (
            ...
            if path.suffix.upper() in {".F", ".F90", ".F95", ".F03", ".F08"}
    ...
    def walk_calls(node, scope_nid: str) -> None:
        ...
        if t == "subroutine_call":
            ...
            if name_node:
                ...
                target_nid = make_id(stem, callee)
                add_edge(
                    scope_nid,
                    target_nid,
                    "calls",
        ...
        if t == "derived_type_definition":
            ...
            if stmt is not None:
                ...
                if name_node is not None:
                    ...
                    type_nid = make_id(stem, type_name)
                    line = node.start_point[0] + 1
