...
def _cross_community_links(G: Any, nodes: list[str], own_cid: int, labels: dict[int, str], node_community: dict[str, int]) -> list[tuple[str, int]]:
...
def _community_article(
    ...
    sources = sorted({node_attributes(G, n).get("source_file") or "" for n in nodes} - {""})
...
def _god_node_article(G: Any, nid: str, labels: dict[int, str], node_community: dict[str, int] | None = None, resolver: dict[str, str] | None = None) -> str:
    resolver = resolver or {}
    d = node_attributes(G, nid)
    node_label = d.get("label", nid)
    src = d.get("source_file", "")
    ...
    lines: list[str] = []
    lines += [f"# {node_label}", ""]
    lines += [f"> God node · {G.degree(nid).degree} connections · `{src}`", ""]
    ...
    for neighbor in sorted(G.neighbors(nid), key=lambda n: G.degree(n).degree, reverse=True):
        nd = node_attributes(G, neighbor)
        ed = edge_data(G, nid, neighbor)
...
def to_wiki(
    ...
    # Filter stale community IDs that no longer exist in the active generation.
    import sys as _sys
    _g_nodes = {node.id for node in G.nodes()}
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `graphify extract .` to regenerate native analysis state."
        )
    ...
    for node_data in god_nodes_data:
        ...
        if nid and G.contains_node(nid):
    ...
    (out / "index.md").write_text(
        _index_md(communities, labels, god_nodes_data, G.node_count, G.edge_count, resolver),
        encoding="utf-8",
    )
