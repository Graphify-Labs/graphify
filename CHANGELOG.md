...
- Fix: the `get_community` MCP tool now shows the community name in its header (`Community 12 — Auth & Sessions (8 nodes)`), matching `get_node` and the query-traversal output, which already read the `community_name` node attribute written by `to_json`. `get_community` was the only graph tool still returning a bare numeric id, so an agent that had located a community by name (via the report, `get_node`, or query output) couldn't confirm it here. The name is read from the community's member nodes — they all share it — and falls back to the bare id for graphs built before `community_name` was persisted; the value still passes through `sanitize_label` like every other LLM-derived field (#XXXX).
