...
- Fix: `graphify extract` no longer writes an empty `{}` manifest after a successful semantic extraction when the LLM sets `source_file` to a repo-relative path but `detect()` records absolute corpus paths. `_manifest_files` matched with plain string equality (`f in _sem_extracted`), so document files were excluded from manifest stamping, breaking incremental mode on the next run. Matching now uses `path_covered_by_extraction()` so absolute detect paths and relative LLM `source_file` values reconcile (#TBD).
