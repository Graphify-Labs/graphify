...
- Fix: `save_manifest` no longer leaves stale rows for files that are still on disk but excluded from a full rescan, which made `graphify update` report permanent false deletions on every incremental run (#1908). Retention previously checked only `Path(f).exists()`, so a physically-present-but-scan-excluded file (ignore rules, generated-file handling) stayed in the manifest forever. `save_manifest` now takes a `prune_to_scan` flag; `watch._rebuild_code`'s full-rebuild call sites pass `prune_to_scan=True` so a manifest entry only survives if it's both on disk and in the current scan. Default stays subset-preserving so incremental callers passing only changed files still don't erase untouched entries (#917).
