...
- Docs: clearer install/PATH guidance for `uv tool install graphifyy` on macOS (#1471, thanks @Patsch36). Two expected uv behaviors were undocumented and read as bugs. (1) After `uv tool install`, the `graphify` command lands in uv's tool bin dir (`~/.local/bin`), which a fresh macOS/zsh shell often doesn't have on `PATH` — so `graphify` came back `command not found`. The README no longer implies uv always wires `PATH` automatically; it points to `uv tool update-shell` (then a new terminal) and `uv tool dir --bin` to locate the dir. (2) `uvx graphify …` / `uv tool run graphify …` resolve the first word as a *package*, so they fail with `No solution found … no versions of graphify` — the package is `graphifyy` and `graphify` is only the console script it provides. The docs now show the explicit form `uvx --from graphifyy graphify install`. README install note and Troubleshooting section only; no code change.
