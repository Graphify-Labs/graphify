...
- Feat: the MCP server can serve many projects from one process via an optional `project_path` on every tool. Omit it and nothing changes — the server answers against the graph it was started with, exactly as before. Pass an absolute `project_path` and that call is routed to `<project_path>/<GRAPHIFY_OUT>/graph.json` instead, with its own mtime+size hot-reload, so a single stdio/HTTP server backs a whole workspace of repos without one process per project. Graphs are loaded lazily and cached per resolved path; a missing/corrupt project graph is returned as a tool error rather than exiting the process, and the server now starts even when its default graph is absent (pure multi-project mode). Backward-compatible and additive: existing single-graph setups and every tool's required fields are untouched.
