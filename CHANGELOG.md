...
- Feat: Solidity ``.sol`` AST extraction via ``tree-sitter-solidity``. Contracts, interfaces, libraries, structs, enums, imports (with Foundry remapping resolution), inheritance/``implements``, events, ``emit``, ``new``/``instantiates``, and call-graph edges are now indexed (#1362).
