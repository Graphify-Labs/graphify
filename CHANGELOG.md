...
- Feat: Sigma.js + graphology fallback for large `graph.html` views, plus batched/reconciled community labeling at scale. vis-network's live client-side forceAtlas2 physics simulation is genuinely slow/laggy past a few hundred nodes regardless of hardware — verified in production on a 1083-community monorepo graph (apa, 2026-07-03). Step 6 of the skill now generates a separate WebGL Sigma.js render (`graph_sigma.html`) with the layout precomputed offline in Python (`nx.forceatlas2_layout`, no client-side physics) once the view exceeds ~300 nodes; the full recipe lives in the new `references/sigma-viz.md`, shipped to every split-platform skill variant. Step 5's community labeling also gains a batched-subagent flow for 100+ communities (sample each community's top-degree nodes, dispatch one `general-purpose` subagent per ~15-20-community batch in parallel) plus a required cross-batch label-reconciliation pass — a naive single-pass label run on the same graph left 338 of 1083 communities (31%) sharing a label with another before reconciliation.
  ...
  Resizing the browser window *after* the page had already loaded (not just choosing a window size before opening it) left the graph and hulls pinned at their original size and position, with dead space filling the rest of the window — sigma's own internal resize listener updates the canvas/WebGL viewport but never re-fits the camera. Two fix attempts made this worse before landing on a robust one. The first called `fitViewportToNodes` synchronously from the resize handler, which read node display data mid-flight through sigma's own pending (asynchronous) re-normalization pass and got back stale, un-normalized graph coordinates, pointing the camera far outside the graph and rendering a blank canvas. The second deferred that same call by one `requestAnimationFrame`, which fixed the common case but merely assumed it would run after sigma's own internal one — it still raced and failed specifically when the window was resized in the first instants after the page loaded, or resized rapidly (a real drag-resize fires many resize events in quick succession, not one at the end). The robust fix ties the re-fit to sigma's own `afterRender` event instead of a guessed frame count: set a flag on `resize`, act on it the next time `afterRender` actually fires, since that event by definition only fires once a render pass — including any pending re-normalization — has completed. The same investigation also explained a related report of the hull overlay and the graph visibly disagreeing with each other after a resize: drawing the hull canvas eagerly right after mutating the camera made it reflect the new state a frame before sigma's own WebGL canvases caught up, since camera/bbox mutations only *schedule* a render rather than repainting synchronously. The hull draw now happens only when there's no pending re-fit, so it is always painted after nodes/edges have already been redrawn with the same camera state. Verified against the real 20,097-node/1083-community production graph: loading then resizing immediately (both larger and smaller), and a simulated rapid multi-step drag-resize, all settle with the graph filling the viewport and hulls staying aligned with nodes throughout — no dead space, no blank canvas, no desync.
  A further node-clumping report against the same production graph led to re-measuring the module-clustering boost against the current layout-spread settings together, rather than in isolation: `MODULE_CLUSTER_BOOST=15` was found to actively backfire on the two largest modules specifically (a big module has proportionally more same-module edges, so the boost compounds), collapsing them into their own dense sub-hairball even though the aggregate within/between-module distance ratio looked fine. Lowering it to 6 (paired with raising `scaling_ratio` from 80 to 100) prioritizes breaking up that hairball: the fraction of nodes within 15% of the bounding-box center on the real graph dropped from 0.28 to 0.12 and the median nearest-neighbor spacing roughly tripled, at the cost of somewhat looser module separation (the hull overlay still visually groups a module even when its nodes aren't packed tightly, so this was judged an acceptable trade for legibility) — confirmed visually, not just by the numeric proxy, per the same lesson learned from the original spread-tuning pass.
  The header now also identifies which repo and branch a given `graph_sigma.html` is of — a repo name, its full scan-root path, and the git branch (best-effort; degrades silently on a non-git corpus or when the scan-root marker is missing) — since nothing on the page previously distinguished one open copy of the file from another, which matters once someone has more than one open (e.g. comparing branches, or a stale copy from a previous run).
  The sidebar's custom scrollbar (added in an earlier pass specifically because macOS auto-hides scrollbars, which made an already-working scroll region look like a dead end) only used `::-webkit-scrollbar-*` rules — a Chromium/WebKit-only proprietary API that does nothing in Firefox. Firefox users got no visual indicator at all and would see the exact same "looks truncated, unscrollable" problem this was meant to fix in the first place. Added the standard `scrollbar-width`/`scrollbar-color` properties alongside the existing WebKit rules so the scrollbar renders (and stays visible, not auto-hidden) across browsers, not just Chromium/Safari.
