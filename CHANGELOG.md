...
- Fix: `SKILL.md`'s uv-based interpreter detection no longer falls back to a system Python that lacks graphify installed (#1735). Step 1 ran `uv tool run graphifyy python -c ...`, but since the installed package name (`graphifyy`) differs from its executable name (`graphify`), uv reads `python` as an argument to a command literally named `graphifyy` and errors out — silently, because the failure was piped to `2>/dev/null`. `_UV_PY` came back empty and detection fell through to a bare `python3`, breaking every subsequent step with `ModuleNotFoundError: No module named 'graphify'`. The invocation is now `uv tool run --from graphifyy python -c ...`, the syntax uv's own error message recommends.
