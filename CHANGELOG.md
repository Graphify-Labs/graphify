...
- Feat: new `graphify resolve-entities <path>` subcommand runs the existing `dedup.deduplicate_entities()` pipeline against an already-built `graph.json`, so users can clean up graphs produced by custom build scripts that bypassed the standard `build()` path, or graphs imported from external sources, without re-extracting from source. Flags mirror `cluster-only`: `--graph <path>`, `--backend <name>` (optional LLM for ambiguous-pair resolution), `--dry-run` (report merges without writing), `--skip-recluster` (don't re-run clustering after the merge). A `.bak` of the original graph.json is written before the rewrite, and the link/edge key is auto-detected so the command works on both NetworkX node-link (`links`) and raw extraction (`edges`) shapes (#new).
