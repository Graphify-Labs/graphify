...
**Curriculum corpora:** If the user identifies the corpus as curriculum, or the
documents contain week/pacing, assignment-alignment, release, or due-date tables,
read `references/curriculum-tables.md`. Apply its authority and identifier rules
during semantic extraction, then run its deterministic normalizer and fail-closed
validator at the gates below. Do not substitute course-specific mappings.
...
#### Part D - Curriculum table normalization (curriculum corpora only)

Run this after Part C and before graph build. It deterministically reads source
tables, adds authority-scoped timing concepts and row-provenance edges, and
replaces LLM timing guesses from those schedule authorities:
...
`SKILL_DIR` is the directory containing this `SKILL.md`. The command is generic:
never pass assignment counts, week counts, filenames, or expected mappings.
...
For a curriculum corpus, now run the table-coverage validator:
...
Unlike the general health diagnostic, this gate is fail-closed. Stop before
labeling/wiki generation if it reports skipped rows, assignments without timing,
missing timing concepts, invented timing, missing provenance, or collapsed
schedule authorities. Report competing authorities as competing schedules, not
...
rm -f graphify-out/.curriculum_table_expectations.json 2>/dev/null || true
