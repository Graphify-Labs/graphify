...
- Curriculum timing tables: treat assignment numbers as identifiers, never weeks. Extract every meaningful assignment-alignment, pacing, release, and due-date row; preserve multiple assignments per period and explicit begin/finish spans. Keep proposed plans, current release schedules, and historical evidence as separate authorities even when their timing differs. Attach row-level source_location. Never convert a date or assignment number into a week unless that source row explicitly states the week. A deterministic curriculum-table normalizer and validator runs after semantic extraction, so preserve the source's authority language and do not collapse competing schedules.
