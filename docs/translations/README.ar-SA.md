...
├── graph.helix       رسم بياني دائم — استعلم بعد أسابيع دون إعادة القراءة
...
Helix + Leiden (native Leiden) + tree-sitter + vis.js. استخراج دلالي عبر Claude أو GPT-4 أو نموذج منصتك. نسخ الفيديو عبر faster-whisper + yt-dlp (اختياري).
